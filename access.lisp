...
(defun config-info-search-name (name &key (namespace :both) (db *default-db*))
  (labels ((name-equal (name config-info)
             (let* ((cf-name (config-info-name config-info))
                    (subname (if (and (> (length cf-name) 12)
                                      (string= (subseq cf-name 0 12)
                                               "config-info-"))
                                 (subseq cf-name 12)
                                 cf-name))
                    (nl (length name))
                    (sl (length subname)))
               (when (<= nl sl)
                 (string-equal name subname :end1 nl :end2 nl)))))
    (list (when (or (eql namespace :both)
                    (eql namespace :accessors))
            (loop for v being the hash-value of (car db)
                  when (name-equal name v)
                    collect v))
          (when (or (eql namespace :both)
                    (eql namespace :variables))
            (loop for v being the hash-value of (cdr db)
                  when (name-equal name v)
                  collect v)))))
...
list of symbols representing an accessor and a place."
  (declare (type (or string list symbol) term)
           ...
           (type cons db))
  (cond ((stringp term)
	 (config-info-search-name term :namespace namespace :db db))
