...
** Defconfig and CLISP
   Many distributions package an older version of CLISP upon which the defconfig dependencies ~:trivial-cltl2~ and ~:fiveam~ wont load. Please clone CLISP from the official gitlab repo and test using that before opening any issues regarding CLISP specific behavior. 
