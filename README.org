...
** Defconfig and CLISP
   While defconfig loads on CLISP, there are some discrepancies. Namely, ~with-atomic-setv*~ is unavailable and the test suite will not load. Any issues with defconfig on CLISP will be considered, but may not be given priority. 
