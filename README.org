...
* Quirks and Oddities
  There are a few places in ~ðefconfig~ that arent naturally intuitive. 
** ~With-atomic-setv~
...
#+BEGIN_SRC lisp
  ...
  (with-atomic-setv* ()
    ...
    …)
#+END_SRC
   Both of these will evaluate of these will evaluate ~(accessor *myvar*)~ multiple times depending on whether it gets reset or not. 
#+BEGIN_SRC lisp
  (with-atomic-setv ()
...
#+END_SRC
   ...
   Both version of this macro will evaluate the accessor multiple times. 
